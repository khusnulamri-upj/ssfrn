...
function GetNextNIM($TahunNIM, $mhsw) {
  ...
  $StatusAwalID2 = ($mhsw['StatusAwalID2'] == 'Y')? '2' : '1';
  ...
  $check2 = substr(trim($check), 0, strlen(trim($check))-4).'____';    
  $xxx2 = $check2;
...
function GetNextNIMSementara($TahunNIM, $mhsw) { 
  ...
  $check = str_replace('~NMR4~', '____', $check);
  $check = str_replace('~NMR5~', '_____', $check);  
  $check2 = substr(trim($check), 0, strlen(trim($check))-4).'____'; // 'eg. 12TEMPTGF____' dan '1208S____'; 
