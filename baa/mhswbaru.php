...
function MhswBaruEdt($gels, $gel) {
  ...
  $pmb = GetFields("pmb 
    ...
    "pmb.KodeID='".KodeID."' and pmb.PMBID", $PMBID,
...
function TampilkanHeaderMhswBaruEdt($gels, $gel, $pmb) {
  ...
  if (empty($pmb['MhswID'])) {
    ...
    if ($pmb['PrintTagihan'] >= 1) {
      ...
      $TombolTambahBipot = "<input type=button name='TambahBipot' value='Tambah Bipot' onClick=\"javascript:BIPOTEdit('$pmb[PMBID]', 1, 0)\" style='color:red;font-weight:bold' />";
    ...
    $Tombol2 = "$TombolProsesBipot
      $TombolHapusSemuaBipot
      $TombolTambahBipot
      <input type=button name='TambahBayar' value='Tambah Pembayaran' onClick=\"javascript:ByrEdit('$pmb[PMBID]', 1, 0, 0)\" style='color:yellow;font-weight:bold' />
    ...
    if ($_SESSION['_LevelID'] == 1 || $_SESSION['_LevelID'] == 70) { // hanya superuser atau ka. keu
      if ($pmb['PrintTagihan'] >= 1) {
        ...
        if ($byrm_jml > $byrm2['Jumlah']) { // Jumlah pembayaran dari gabungan dua biaya, tidak bisa edit tapi bisa delete
          ...
          $edit = "<a href='#' onClick=\"javascript:BIPOTEdit('$pmb[PMBID]', 0, $w[BIPOTMhswID])\"><img src='img/edit.png' /></a>";
        }         
